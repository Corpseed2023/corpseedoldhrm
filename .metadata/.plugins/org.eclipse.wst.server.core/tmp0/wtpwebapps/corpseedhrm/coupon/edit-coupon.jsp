<%@page import="admin.coupon.Coupon_ACT"%>
...
String[][] product=TaskMaster_ACT.getCouponByKey(uuid,token);
...
<input type="hidden" name="couponUuid" value="<%=uuid%>">
<div class="col-md-12">	
<div class="row mb-10">
   <div class="col-md-6">
    <label for="exampleInputEmail1" class="form-label">Coupon title</label>
    <input type="text" class="form-control" value="<%=product[0][2] %>" onchange="isCouponExist(this.value)" id="couponTitle"name="couponTitle" aria-describedby="emailHelp" placeholder="Enter Coupon title......">
  ...
  <div class="col-md-6">
    ...
    <input type="text" class="form-control" id="couponValue" value="<%=product[0][3] %>" name="couponValue" placeholder="Enter Coupon discount value...." onkeypress="return isNumberKey(event)">
  </div>
 </div> 
 </div>
<div class="col-md-12">	
...
<div class="col-md-2">
  <input class="form-check-input" type="radio" name="couponType" value="percentage" id="flexRadioDefault1" <%if(product[0][4].equals("percentage")){ %> checked<%} %>>
  Percentage
    </div>
<div class="col-md-8">  
  <input class="form-check-input" type="radio" name="couponType" value="fixed" id="flexRadioDefault2" <%if(product[0][4].equals("fixed")){ %> checked<%} %>>
  Fixed
  </div></div>
 </div>
 <div class="col-md-12 max-discount <%if(product[0][4].equals("fixed")){%>noDisplay<%}%>">	
<div class="row mt-10 mb-10">
   <div class="col-md-6">
    <label for="exampleInputEmail1" class="form-label">Maximum Discount</label>
    <input type="text" class="form-control" value="<%=product[0][11] %>" id="MaxDiscount" name="maxDiscount" aria-describedby="emailHelp" placeholder="Enter maximum discount......" onkeypress="return isNumberKey(event)">
  </div>    	 	
 </div> 
 </div>
 <div class="col-md-12">	
...
<div class="col-md-2">
  <input class="form-check-input" type="radio" name="serviceType" value="selected" id="flexRadioDefault3" <%if(product[0][13].equals("selected")){ %> checked<%} %>>
  Selected
    </div>
<div class="col-md-8">  
  <input class="form-check-input" type="radio" name="serviceType" value="all" id="flexRadioDefault4" <%if(product[0][13].equals("all")){ %> checked<%} %>>
  All
  </div></div>
 </div>
 
<div class="col-md-12 select-service <%if(product[0][13].equals("all")){%>noDisplay<%}%>">	
<div class="row mt-10 mb-10">
   <div class="col-md-12">
...
<div class="row mt-10 mb-10">
    ...
    <input type="date" class="form-control" value="<%=product[0][5] %>" id="StartDate" name="startDate" placeholder="Enter Coupon start date......">
   </div>
  <div class="col-md-6">
    <label for="exampleInputPassword1" class="form-label">End date</label>
    <input type="date" class="form-control" value="<%=product[0][6] %>" id="EndDate" name="endDate" placeholder="Enter Coupon end date....">
  </div>
...
<div class="col-md-2">
  <input class="form-check-input" type="radio" name="displayststus" value="1" id="flexRadioDefault5" <%if(product[0][7].equals("1")){ %> checked<%} %>>
  Enable
    </div>
<div class="col-md-8">  
  <input class="form-check-input" type="radio" name="displayststus" value="2" id="flexRadioDefault6" <%if(product[0][7].equals("2")){ %> checked<%} %>>
  Disable
  </div></div>
 </div>
...
$(document).ready(function(){
	...
	getSelectedService("<%=uuid%>");
});
function getSelectedService(uuid){
	$.ajax({
		type : "POST",
		url : "GetSelectedService111",
		dataType : "HTML",
		data : {uuid : uuid},
		success : function(response){
			if(Object.keys(response).length!=0){
				...
				var value=[]; 
				for(var i=0;i<len;i++){
					var prodNo = response[i]['productNo'];
					value.push(prodNo);									
				}
				$("#Services").val(value);	
