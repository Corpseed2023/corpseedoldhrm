...
<div class="container">
<%
String genKey=request.getParameter("uid").trim();
genKey=genKey.substring(16,genKey.length()-5); 
String billData[][]=Enquiry_ACT.getGeneratedEstimateData(genKey);
String client[]=null;
if(billData!=null&&billData.length>0){
	String clientKey=Enquiry_ACT.getEstimateClientKey(billData[0][2]);
	client=Enquiry_ACT.getClientsDetail(clientKey);
...
<span>Noida, Uttar Pradesh - 201301</span><br/><%if(gstStatus){ %>
...
<p style="font-weight:600;" id="EstimateBillNo"># <%=billData[0][8]%></p>
</div>
</div>
</div>
<div class="clear"></div>
...
<p><span style="font-weight:600;color:#888;">Date :</span> <span style="padding-left:20px;" id="EstimateDate"><%if(billData!=null){ %><%=billData[0][5]%><%} %></span></p>
</div>

</div>
<div class="clear"></div>
<div class="table-responsive">
<table  border="1" style="width:100%;border:1px solid #7c7272">
...
<tr><td style="padding: 3px;font-weight:600">1</td>
<td style="padding: 3px;font-weight:600" colspan="7"><%=billData[0][3] %></td></tr>
<%
double rate=0;
double gstSum=0; 
String estimateDetails[][]=Enquiry_ACT.getGeneratedEstimateDetails(genKey);
if(estimateDetails!=null&&estimateDetails.length>0){
	for(int j=0;j<estimateDetails.length;j++){
		double gst=(Double.parseDouble(estimateDetails[j][4])*(Double.parseDouble(estimateDetails[j][6])+Double.parseDouble(estimateDetails[j][5])+Double.parseDouble(estimateDetails[j][7])))/100;
		rate+=Double.parseDouble(estimateDetails[j][4]);
		gstSum+=gst;
%>
<tr>
<td></td>
<td style="padding: 3px;"><%=estimateDetails[j][3] %></td>
<td style="padding: 3px;"><%=CommonHelper.withLargeIntegers(Math.round(Double.parseDouble(estimateDetails[j][4]))) %></td>
<td style="padding: 3px;"><%=estimateDetails[j][6] %>%</td>
<td style="padding: 3px;"><%=estimateDetails[j][5] %>%</td>
<td style="padding: 3px;"><%=estimateDetails[j][7] %>%</td>
<td style="padding: 3px;"><%=CommonHelper.withLargeIntegers(Math.round(gst)) %></td>
<td style="padding: 3px;"><%=CommonHelper.withLargeIntegers(Math.round((Double.parseDouble(estimateDetails[j][4])+gst)))%></td>
</tr>
<%}} %>
...
<td style="padding: 3px;"><%=CommonHelper.withLargeIntegers(Math.round(Double.parseDouble(billData[0][4])))%></td>
</tr>
</table>
</div>
<div class="clear"></div>
...
<%if(billData[0][9]!=null&&!billData[0][9].equalsIgnoreCase("NA")){%>
<p style="font-size: 11px;color:#888;"><%=billData[0][9] %></p>
<%} %>
</div>
<div class="clearfix" style="width:100%;">
<p style="color:#888;">
...
$( document ).ready(function() {
	var totalAmount="<%=Math.round(Double.parseDouble(billData[0][4]))%>";
