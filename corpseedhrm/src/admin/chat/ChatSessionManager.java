...
//    	System.out.println("message.getSendTo()=="+message.getSendTo());
        SESSIONS.stream().filter(session -> !session.equals(origin)).forEach(session -> {
            try {
//            	System.out.println("session.getUserProperties().get(Constants.USER_NAME_KEY)="+session.getUserProperties().get(Constants.USER_NAME_KEY)+"/"+message.getSendTo());
            	if(session.getUserProperties().get(Constants.USER_NAME_KEY).equals(message.getSendTo())) {
