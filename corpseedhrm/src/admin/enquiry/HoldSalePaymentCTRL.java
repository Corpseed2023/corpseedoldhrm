...
public class HoldSalePaymentCTRL extends HttpServlet {
...
try{		
	boolean status=false;
		...
		status=Enquiry_ACT.updateBillingHoldAmount(invoiceno,amount,token);
		//hold payment
		status=Enquiry_ACT.holdPayment(refid,loginuaid,today,token,comment);
		
		if(status){
			//adding notification
			String nKey=RandomStringUtils.random(40,true,true);			
			String userName=Usermaster_ACT.getLoginUserName(loginuaid, token);			
			String message="Payment of rs. "+amount+" is <b class='text-danger'>on-hold</b> against sales invoice :"+invoiceno+" by &nbsp;<span class='text-muted'>"+userName+"</span>";
			...
			String message1="<table border=\"0\" style=\"margin:0 auto; width:700px;min-width:700px;font-size:20px;line-height: 20px;border-spacing: 0;font-family: sans-serif;\">"
					...
					+ "                     <p> Payment of Rs. <b>"+amount+"</b> added on Invoice/Estimate <b>"+invoiceno+"</b> is <span style=\"color:#d91f16\">On-Hold</span>, For any query plase contact account department.."
					+ "                      </p></td></tr>"
					...
					+ "                <p>Address:Corpseed, A-43, 2nd Floor, Sector 63 Noida, 201301</p></td></tr></table>";
			
			if(email!=null&&email.length()>0&&!email.equalsIgnoreCase("NA"))
				Enquiry_ACT.saveEmail(email,"empty","Payment On-Hold | Corpseed", message1,2,token);
